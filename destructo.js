...
// TODO: To open the Mysterious Door, a sequence is required, which might be incomplete. Use object destructuring to assign 
//default values to ensure the door opens even if part of the code is missing. Make sure the `middle` defaults to one of the values 
//from the `remaining` variable above if not provided. Print the door code sequence.
...
console.log(`key elemets are: ${one},${two}`);
...
console.log(`the visibleone: ${visibleShadow}, hiddenones: ${hiddenShadows}`);
...
revealPath({direction:'east',distance:25});
...
function mixPotion({ingredient1='Water', ingredient2='Fireflower'}){
	console.log(`basic mixing are ${ingredient1} and ${ingredient2}`);
}
mixPotion({});
...
function castSpell([firstWord,secondWord]){
	console.log(`castSpell: ${firstWord}${secondWord}`);
}
castSpell(['abara','doobie','rooke']);
